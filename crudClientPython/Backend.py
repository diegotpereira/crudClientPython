...
class TransactionObject():
    ...
    def connect(self):
        ...
        TransactionObject        = TransactionObject.conn.cursor()
...
def initDB():
    ...
    trans.connect()
    trans.execute("CREATE TABLE IF NOT EXISTS cliente (id integer PRIMARY KEY, nome TEXT, sobrenome TEXT, email TEXT, cpf TEXT)")
    trans.persist()
    trans.disconnect()
...
def search(nome="", sobrenome="", email="", cpf=""):
    trans = TransactionObject()
    trans.connect()
    trans.execute("SELECT * FROM clientes WHERE nome=? or sobrenome=? or cpf=?", (nome, sobrenome, email, cpf))
    rows = trans.fetchall()
    return rows
...
def update(id, nome, sobrenome, email, cpf):
    trans = TransactionObject()
    trans.connect()
    trans.execute("UPDATE clientes SET=?, sobrenome=?, email=?, cpf=? WHERE id=?", (nome, sobrenome, email, cpf, id))
    trans.persist()
    trans.disconnect()
